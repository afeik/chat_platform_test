...
def api_init_conversation(req: InitConversationRequest):
    db = SessionLocal()
    print(chatbot_config["claude_model"])
    print(f"{chatbot_config['background_questions']['decision']}",    
            f"{chatbot_config['background_questions']['yes']['questions']} ")
    try:
        ...
        user_decision = req.usecase_specific_info.get(chatbot_config["background_questions"]["decision"], "no")
        fine_tuned_prompt = (
            f"{chatbot_config['background_questions']['decision']} - {user_decision}"
            f"Based on this, emphasize these questions (concise and not all at once):"
            f"{chatbot_config['background_questions'][user_decision]['questions']} "
        ...
        response = claude_client.messages.create(
            model=chatbot_config["claude_model"],
            ...
            messages=[{"role": "user", "content": fine_tuned_prompt}],
        )
